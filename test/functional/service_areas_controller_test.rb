...
class ServiceAreasControllerTest < ActionController::TestCase
  ...
  context "A user that is a creator and has been assigned to a directorate that is retired" do
    setup do
      ...
      directorates(:directorates_retired).update_attributes!(:creator_id => @creator.id)
    end
  ...
  context "A user that is a creator and has been assigned to a directorate that is active" do
    setup do
      sign_in @creator = Factory(:creator)
      directorates(:directorates_001).update_attributes!(:creator_id => @creator.id)
    end
    
    should "be able to access the Service Areas table" do
      get :index
      assert_response :success
    end
    
    should "be able to access the New Service Area page" do
      get :new
      assert_response :success
    end
    
    should "be able to create new Service Areas" do
      post :create, :service_area => { :name => "testarea", :approver_email => "shaun@27stars.co.uk", :retired => "0" }
      assert ServiceArea.where(:name => "testarea").first
      assert_response :redirect
      assert_redirected_to service_areas_path
      assert_equal "testarea", ServiceArea.where(:name => "testarea").first.name
    end
    
    should "be able to access the Edit Service Area page" do
      get :edit, :id => 1
      assert_response :success
    end
    
    should "be able to update existing Service Areas" do
      post :update, :id => 1, :service_area => {:name => "Edited Service Area"}
      assert_response :redirect
      assert_redirected_to service_areas_path
      assert_equal ServiceArea.find(1).name, "Edited Service Area"
    end
    
    should "be able to toggle the retired status of Service Areas" do
      post :toggle_retired_status, :id => 1, :checkbox => "retired"
      assert_response :success
      assert_equal true, ServiceArea.find(1).retired
    end
