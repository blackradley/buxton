    ...
    if @current_user.nil?
      ...
      when 'DirectorateManager'
        generate_menu( [
                        { :text => 'Summary',
                          :url => { :controller => 'activities', :action => 'summary' },
                          :title => "#{@current_user.organisation.directorate_term} Control Page - Summary",
                          :status => '' },
                        { :text => 'Activities',
                          :url => { :controller => 'activities', :action => 'list' },
                          :title => "#{@current_user.organisation.directorate_term} Control Page - Activities",
                          :status => '' },
                        { :text => 'Sections',
                          :url => { :controller => 'sections', :action => 'list', :id => 'purpose' },
                          :title => "#{@current_user.organisation.directorate_term} Control Page - Sections",
                          :status => '',
                          :highlight_on => [  { :controller => 'sections', :action => 'list', :id => 'impact' },
                                              { :controller => 'sections', :action => 'list', :id => 'consultation' },
                                              { :controller => 'sections', :action => 'list', :id => 'additional_work' },
