class User < ActiveRecord::Base
  ...
  scope :creator, -> {where "creator is true and type is null and retired is not true"}
  ...
  def reset_password!
    ...
    self.password_confirmation = @new_pass
    self.save
    Mailer.password_reset(self, @new_pass).deliver_now
  end
  ...
  def directorate
    Directorate.find_by(creator_id: self.id)
  end

  def send_password
    Mailer.new_account(self, @new_pass).deliver_now
  end
