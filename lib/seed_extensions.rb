...
module Seed
  ...
  def change_seed(class_name, variables, &block)
    ...
    print variables.to_a.map{|s| s.join(" => ")}.to_sentence
    object_to_create = find_class class_name
    instance_of_object = object_to_create.find_by(variables) #send("find_by_#{variables.keys.join('_and_')}".to_sym, variables.values)
    possible_parent_list = possible_parents instance_of_object
    parent_instance = self unless possible_parent_list.select{|parent| parent == self.class.name}.blank?
    instance_of_object.send("#{parent_instance.class.name.downcase.to_sym}=", parent_instance) if parent_instance
    instance_of_object.instance_eval(&block) if block
  ...
  def remove_seed(class_name, variables)
    ...
    to_delete = object_to_remove.find_by(variables)#send("find_by_#{variables.keys.join('_and_')}".to_sym, variables.values)
  ...
  def has_parent(class_name, variables)
    object_to_create = find_class class_name
    parent_instance = object_to_create.find_by(variables)#send("find_by_#{variables.keys.join('_and_')}".to_sym, variables.values)
