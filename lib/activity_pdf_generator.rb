...
class ActivityPDFGenerator
  ...
  def build_body(pdf, activity)
    ...
    table << ["<b>Reference Number</b>", "#{activity.ref_no.titlecase}"]
  ...
  def get_comments_and_notes(activity, question)
  ...
  def build_purpose(pdf, activity)
    ...
    pdf.text " "
    target_q = activity.question_wording_lookup('purpose', 'overall', 2)[0].to_s
    ...
    impact_answers = []
    (5..9).each do |i|
      impact_quns << ["purpose_overall_#{i}".to_sym, activity.question_wording_lookup(:purpose, :overall, i)[0].to_s]
    ...
    impact_quns.each_with_index do |question, i|
      ...
      comments = get_comments_and_notes(activity, question[0])
  ...
  def build_strand_tables(pdf, activity)
    ...
    bad_impact_questions.each do |question, response|
      ...
      section_strand = question.to_s.split('_')
      question_text = activity.question_wording_lookup(section_strand.first, strand, section_strand.last)[0].to_s
      bad_differentials[strand] = [question, question_text, activity.hashes['choices'][1][activity.send(question).to_i].to_s]
    end
...
#    good_impact_questions.reject!{|question, response| response.to_i <= 1}
  ...
  def build_impact(pdf, activity, strand)
...
#    table_info = build_table_data(activity, collected_information + planned_information, 2, ["impact_","_1"])
    ...
    information = remove_unneeded(activity, information)
    ...
    information.each do |question, answer|
      ...
      question_text = activity.question_wording_lookup('impact', strand, q_no)[0].to_s
      ...
      cell_formats << [{:shading => SHADE_COLOUR}, nil]
      comments = get_comments_and_notes(activity, question)
...
#    specific_data[:cell_format] = table_info[1]
  ...
  def build_consultation(pdf, activity, strand)
    ...
    information.each do |question, answer|
      q_no = question.to_s.slice(/\d+\z/)
      question_text = activity.question_wording_lookup('consultation', strand, q_no)[0].to_s
  ...
  def build_additional_work(pdf, activity, strand)
    ...
    information.each do |question, answer|
      q_no = question.to_s.slice(/\d+\z/)
      question_text = activity.question_wording_lookup('additional_work', strand, q_no)[0].to_s
  ...
  def remove_unneeded(activity, question_array)
    return question_array.select do |question, response|
      (activity.questions.find_by_name(question.to_s) && activity.questions.find_by_name(question.to_s).check_needed)
    end
  end
